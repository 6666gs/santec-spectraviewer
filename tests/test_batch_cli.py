...
def test_cli_csv_keeps_all_despite_low_threshold(tmp_path):
    from batch_ring_q import main
    lam = np.arange(1500, 1600, 0.002)
    t_lin = np.ones_like(lam)
    for c0 in np.arange(1504, 1596, 9.0):
        t_lin *= (1 - 0.7 / (1 + ((lam - c0) / 0.03) ** 2))  # ER0.7 -> Qi/Ql≈1.83
    t_db = 10 * np.log10(np.clip(t_lin, 1e-9, None))
    d = tmp_path / "data"
    d.mkdir()
    _write_two_col(d / "ringL_loss.csv", lam, t_db)
    out = tmp_path / "out"
    # 阈值 1.2 会把所有峰排除出统计，但 CSV 必须保留全部数据
    rc = main([str(d), "--out", str(out), "--min-r2", "0.8", "--max-qi-ql", "1.2"])
    assert rc == 0
    with open(out / "ringL_loss_results.csv") as f:
        rows = list(csv.DictReader(f))
    assert 'qi_ql' in rows[0]
    assert len(rows) >= 10                                  # CSV 未因阈值删行
    assert all(float(r['qi_ql']) > 1.2 for r in rows)      # 全部超过阈值
