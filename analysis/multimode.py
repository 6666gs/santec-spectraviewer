...
DEFAULT_MAX_QI_QL = 20.0   # 统计时 Qi/Ql 上限；高于此值的峰不纳入统计（默认 20）
...
def qi_ql_ratio(fit) -> float:
    """Qi/Ql 比值（Ql<=0 时返回 inf）。"""
    return fit.qi / fit.ql if fit.ql > 0 else float('inf')


def stats_fits(fits, max_qi_ql: float = DEFAULT_MAX_QI_QL):
    """统计用子集：剔除 Qi/Ql 高于阈值的峰（不改变 CSV/原始拟合列表）。"""
    return [f for f in fits if qi_ql_ratio(f) <= max_qi_ql]
